...
export default (req, res) => {
  ...
  const token = cookies.get('jwtUser') || null;
  ...
  if (!requestType || !requestUrl) {
    return res.status(400).json({
      status: 'fail',
      message:
        "Please also submit 'requestType' and 'requestUrl' to this API endpoint!",
    });
  }
  ...
  return axios({
    ...
    .catch((err) => {
      console.log(err);
      return res.status(err.response.data.error.statusCode).json({
